...
ha.2 <- HeatmapAnnotation(group = df.t[-c(1,2,13,30:36),3], col = an.col,
                          annotation_legend_param = list(
                            group = list(title = ""))
)
...
metadata$group <- df.t[-c(1,2,13,30:36),3] #!test & QC and P2
...
plot_loading <- a%>%
  filter( PC1< (min(a$PC1) + ((max(a$PC1) - min(a$PC1)) * 0.01)) | PC1>(max(a$PC1) - ((max(a$PC1) - min(a$PC1)) * 0.1))) %>%
