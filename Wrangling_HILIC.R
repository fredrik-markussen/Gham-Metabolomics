...
hilic.df<-read.csv("./data/230314.CD_compounds_HILIC_Fredrik_csv.csv", sep="")
str(hilic.df)
...
#readr::write_delim(df_PR, "./data/df_HILIC_neg_peakrated_20231218_FM.txt")
...
#write file
readr::write_delim(df_MA[,-26], "data/df_HILIC-neg_metaboanalyst_20231218_FM.txt")



##Convert to long format 
df_long <- df_PR %>% 
  select(species , contains("P"),contains ("qc"), -contains("PR"), blank, tags)

#store ID as vector
ids <- names(df_long[,-1])

#Melt into long format and integrety check
melt_df <- reshape2::melt(df_long, id.vars = c("species", 'tags'))
...
#renam cols
names(melt_df)<- c("species", "tags" ,"sample", "norm_area", "group")
...
#write to disk
write.csv(t_df, "./data/long_format_HILIC-neg_FM_20231218.csv")
